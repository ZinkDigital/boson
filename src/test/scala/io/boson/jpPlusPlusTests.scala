...
class jpPlusPlusTests extends FunSuite {
  ...
  test("Inj .key1..key2"){//No Change is perform, because not all values are of the same type
    val expression: String =  ".Store..Price"
  ...
  test("Inj .key1..key2[@elem]"){//No Change is perform, because not all values are of the same type
  val expression: String =  ".Store..SpecialEditions[@Price]"
...
println("injFuture="+new String(injFuture.join()))
  ...
  } //TODO:implement the '..*', this is not correct
  ...
  }//TODO:implement the '..*', this is not correct l(ist.size==1 but double dot.)
...
var i:Int = 0
  ...
  val arr222: BsonArray = new BsonArray().add(obj333).add(obj333)//put("Store",new BsonObject())
