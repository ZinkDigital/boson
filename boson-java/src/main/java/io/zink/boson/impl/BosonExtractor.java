...
public class BosonExtractor<T> implements Boson {
    ...
    public BosonExtractor(String expression, Consumer<T> _extractFunction) {
        ...
        if (retainedClassOpt.isDefined()) {
            ...
            interpreter = new Interpreter<>(expression, Option.empty(), Option.empty(), Option.apply(anon), Option.apply(typeCase), Option.empty());
        } else {
            typeIsClass = false;
            interpreter = new Interpreter<>(expression, Option.empty(), Option.empty(), Option.apply(anon), Option.empty(), Option.empty());
        }
    }
    ...
    private Object runInterpreter(String bsonEncoded) {
        return interpreter.run(Right$.MODULE$.apply(bsonEncoded));
    }
