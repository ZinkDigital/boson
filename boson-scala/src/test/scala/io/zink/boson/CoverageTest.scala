...
//  }
  ...
  test("ReadMe.md test") {
    ...
    val arrayObj = new BsonArray().add(fieldObj).add(fieldObj)
    val bson = new BsonObject().put("name","SomeName").put("array", arrayObj).encode.getBytes
    val json = """{"name":"SomeName","array":[{"field1":10,"field2":20},{"field1":11,"field2":21},{"field1":12,"field2":22}]}"""

//    println(json.getBytes().mkString(", "))

    val ex = ".array[1]"
    val jsonInj = Boson.extractor(ex, (in: Array[Byte]) => {
      println(in.mkString(", "))
    })

    val future = jsonInj.go(bson)
    val resultValue = Await.result(future, Duration.Inf)
