...
class ExtractorTests extends FunSuite {
  ...
  test("Extract BsonArray") {
    val bosonBson: Boson = new Boson(byteArray = Option(globalObj.encode().getBytes()))
    assert(Map("André" -> 975, "António" -> 975) === bosonBson.extract(bosonBson.getByteBuf, "Colleagues", "first").get.asInstanceOf[List[Any]].head)
  }
  ...
  test("Extract deep layer") {
    ...
    val bosonBson: Boson = new Boson(byteArray = Option(bsonEvent.encode().getBytes()))
    assert("Day3" === bosonBson.extract(bosonBson.getByteBuf, "JoséMonthLeave", "first").get.asInstanceOf[List[Any]].head)
  }
