...
//  }
  ...
  test("Extract Json .Epoch") {
    val expression: String = ".Epoch"
    val joson = Joson.extractor(expression, (in: Int) => {
      assertEquals(3, in)
      println(s"in: $in")
      println("APPLIED")
    })

val r = new JsonObject(jsonObj.encode()).encode()
    //val r = new JsonObject("""{"Epoch":3}""").encode()
    println(r)
    val res = joson.go(r)
    Await.result(res, Duration.Inf)
  }
