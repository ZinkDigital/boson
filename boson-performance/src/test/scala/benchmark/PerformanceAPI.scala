...
//  performanceJsonPath("$.Markets[10].selectiongroupid", "")
  ...
  private def performanceJsonPath(path: String, value: Any): Unit = {
    (0 to CYCLES).foreach(_ => {
      val start = System.nanoTime()
      val jsonParse = Lib.bson.asJson().toString
      var aux: Any = JsonPath.using(conf2).parse(jsonParse).read(path) // this is technically also incorrect because it maps the path twice and runs twice through the bson
      aux = value
      JsonPath.using(conf2).parse(jsonParse).set(path, aux)
      val end = System.nanoTime()
      timesBuffer.append(end - start)
    })
