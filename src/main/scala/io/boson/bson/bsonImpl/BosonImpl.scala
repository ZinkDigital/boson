  ...
  private def extractFromBsonArray(netty: ByteBuf, length: Int, arrayFRIdx: Int, keyList: List[(String, String)], limitList: List[(Option[Int], Option[Int], String)]): Iterable[Any] = {
    keyList.head._1 match {
      case EMPTY_KEY if keyList.size < 2  && keyList.head._2.equals(C_LIMIT)=> // case expression = ..[#] only!!
        println("emptyKey, keylist.size = 1, condition LIMIT")
...
//                            case Some(x) if x.isEmpty => None // indexOutOfBounds treatment
//                            case Some(x) => Some(x.toVector)
//                          } TODO: removed this but needs to be more tested
                        case _ =>
                      ...
                      arrayFRIdx - finishReaderIndex match {
                        case 1 =>
                          keyList.head._1 match {
                            ...
                            case _ if keyList.head._2.equals(C_NEXT) | keyList.head._2.equals(C_LEVEL) => errorAnalyzer(limitList.head._3) //TODO: test the c_next case
                            case _ =>
...
//                          Some(arr)
  ...
  private def findElements(netty: ByteBuf, nettyUntouched: ByteBuf, keyList: List[(String,String)], limitList: List[(Option[Int], Option[Int], String)],start: Int, finish: Int): Iterable[Any] = {
    ...
    /*finalValue match {
      ...
      case Some(value) if keyList.head._2.equals(C_LIMIT) =>
        println("Some, condition limit")
        actualPos2 match {
          case x if x > 0 =>
            println("not over")
            Some(value) ++ findElements(netty,nettyUntouched,keyList,limitList,start,finish)
          case 0 =>
            println("over")
            Some(value)
        }
      ...
      case KeyWithArrExpr(key: String, arrEx: ArrExpr) =>
        val input: (String, Int, String, Any) =
          (arrEx.leftArg, arrEx.midArg, arrEx.rightArg) match {
            case (i, o1, o2) if o1.isDefined && o2.isDefined  => (key, arrEx.leftArg, o1.get, o2.get)
            case (i, o1, o2) if o1.isEmpty && o2.isEmpty => (key, arrEx.leftArg, TO_RANGE, arrEx.leftArg)
            case (0, str, None) =>
              str.get match{
                case "first" =>(key, 0, TO_RANGE, 0)
                case "end" =>(key, 0, C_END, None)
                case "all" =>(key, 0, TO_RANGE, C_END)
              }
          }
