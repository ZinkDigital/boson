...
class injectorAPITests extends FunSuite {
  ...
  /*test("simple test"){
    ...
    val bsonArray = bson.encodeToBarray()

    val boson: Boson = Boson.injector(expression, (in: String) => in)
    boson.go(bsonArray)
...
//    println("Result:   "+result+"\nExpected: "+expected)
  ...
  test("extract Key.Key[@elem]") {
    ...
    val boson: Boson = Boson.extractor(expression,(out: Seq[Array[Byte]]) => future.complete(out))
    boson.go(validatedByteArr)
    val expected: Seq[Array[Byte]] = Seq(br1.getBsonObject(0).encodeToBarray(),br1.getBsonObject(2).encodeToBarray())
    val result = future.join()
    assert(expected.size === result.size)
    assertTrue(expected.zip(result).forall(b => b._1.sameElements(b._2)))
  }
  ...
  test("extract Key.Key[#to#].Key[#to#]") {
    ...
    val expected: Seq[Array[Byte]] = Seq(b10.encodeToBarray())
    val result = future.join()
    assert(expected.size === result.size)
    assertTrue(expected.zip(result).forall(b => b._1.sameElements(b._2)))
  }
  ...
  test("extract Key.Key[#].Key[@elem]") {
    ...
    val expected: Seq[Array[Byte]] = Seq(br3.getBsonObject(0).encodeToBarray())
    val result = future.join()
    assert(expected.size === result.size)
    assertTrue(expected.zip(result).forall(b => b._1.sameElements(b._2)))
  }
  ...
  test("extract Key.Key[@*elem].Key[#]") {
    ...
    val expected: Seq[Array[Byte]] = Seq(br4.getBsonObject(0).encodeToBarray(),br5.getBsonObject(0).encodeToBarray())
    val result = future.join()
    assert(expected.size === result.size)
    assertTrue(expected.zip(result).forall(b => b._1.sameElements(b._2)))
  }
