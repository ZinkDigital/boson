    ...
    def constructWithLimits(iter: Int): Iterable[Any] = {
      ...
      val newSeq: Option[Any] =
        seqType2 match {
                ...
                limitList.head._1 match {
                  ...
                  case None =>
                    keyList.head._2 match {
                      case C_FILTER =>
                        ...
                        findElements(copyNetty,keyList,bsonFinishReaderIndex) match {
                          case Seq() =>
                            ...
                            None
                          case _ =>
                            val res: Map[Any, Any] = traverseBsonObj(netty,map,bsonFinishReaderIndex,List((keyList.head._1,C_ALL)),List((None,None,EMPTY_RANGE)))  //TODO:refactor this section
                      ...
                      case _ => Some(traverseBsonObj(netty, map, bsonFinishReaderIndex, keyList,List((None,None,EMPTY_RANGE))))
                    }
                }
            }
          case D_BSONARRAY =>
            ...
            if(!keyList.head._2.equals(C_FILTER)) {
              limitList.head._2 match {
                case Some(_) if iter >= limitList.head._1.get && iter <= limitList.head._2.get =>
                  keyList.head._2 match {
                    case C_LEVEL => Some(extractFromBsonArray(netty, valueLength2, finishReaderIndex, keyList, limitList.drop(1)))
                    case _ => Some(traverseBsonArray(netty, valueLength2, finishReaderIndex, keyList, List((None,None,EMPTY_RANGE))).toArray[Any])
                  }
                case Some(_) =>
          ...
          case D_LONG =>
            val value: Long = netty.readLongLE()
            if(!keyList.head._2.equals(C_LEVEL)) {
              limitList.head._2 match {
                case Some(_) if iter >= limitList.head._1.get && iter <= limitList.head._2.get =>
                  keyList.head._2 match {
                    case C_LEVEL => None
...
//    println(s"limitList: $limitList")
              ...
              * */
              ...
              if(list.size==1){
                ...
                if(list.head._2.contains("..")){
                  ...
                  val b = buffer.readBytes(size)

                  val res = execStatementPatternMatch(b, list.drop(1), f)
                  val wahtever = execStatementPatternMatch(res.duplicate(), list, f)
                  result.writeBytes(wahtever)
                }else{
            ...
            case D_BSONOBJECT =>
              ...
              if(hasElem){
                if(list.size==1/*Dimensao 1*/) {
                  ...
                  if(list.head._2.contains("..")){
                    val size = buf.getIntLE(buf.readerIndex())
                    val b = buf.readBytes(size)

                    val res = execStatementPatternMatch(b, list.drop(1), f)
                    val whatever = execStatementPatternMatch(res.duplicate(), list, f)
                    result.writeBytes(whatever)
  ...
  def execStatementPatternMatch[T](buf: ByteBuf, statements: List[(Statement,String)], f :Function[T,T], result:ByteBuf=Unpooled.buffer()): ByteBuf = {
    ...
    //val newStatementList: List[(Statement, String)] = statements.get.drop(1)
